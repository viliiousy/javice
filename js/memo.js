...
const Memo = {
  ...
  render() {
    ...
    wrap.innerHTML = items.map(m => `
      <div class="memo-item" draggable="true"
  ...
  _saveEdit(id) {
    ...
    if (m) { m.title=title; m.content=document.getElementById('mEditContent')?.value.trim()||''; m.updatedAt=new Date().toISOString(); }
    this.saveItems(items);
    this.render();
    App.closeModal();
    App.showToast('수정됨 ✓','success');
  },
