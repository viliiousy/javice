// js/calendar-ui.js — 미니 달력 (long press → 추가 메뉴)
...
const CalendarUI = {
  ...
  render(container, currentDate, events, selectedDate) {
    ...
    for(let d=1;d<=daysInMo;d++){
      ...
      cells+=`<div class="cal-day${isToday?' today':''}${isSel&&!isToday?' selected':''}${evDays.has(d)?' has-event':''}${clDays.has(d)?' has-cl':''}"
  ...
  _startLP(yr,mo,day,e) {
    ...
    this._lpTimer=setTimeout(()=>{
      CalendarUI._endLP();
      App.showLongPressMenu(new Date(yr,mo,day));
    },600);
  },
