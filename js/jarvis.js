// js/jarvis.js — ⚡ JARVIS (전체 액션: 할일·캘린더·체크리스트·습관·식단·메모)

const JARVIS = {
  history:[], recognition:null, synthesis:window.speechSynthesis,
  listening:false, isOpen:false, _proactiveShown:false,
      ...
      <div id="jarvisPanel" class="jarvis-panel hidden">
        ...
        <div id="jMsgs" class="j-msgs">
          <div class="j-msg j-ai"><div class="j-avatar">⚡</div>
            <div class="j-bubble">JARVIS입니다. 무엇이든 말씀해주세요!<br>
              <span style="font-size:11px;color:var(--accent-l)">할일·일정·체크리스트·습관·식단·메모 추가/삭제/수정 가능<br>📷 사진 버튼으로 식단 자동 분석</span>
            </div>
          </div>
        </div>
  ...
  _setupListeners() {
    setTimeout(()=>{ const inp=document.getElementById('jInput'); if(inp) inp.onkeypress=e=>{ if(e.key==='Enter') this.send(); }; },200);
  },
    ...
    try {
      switch(action.type) {
        ...
        case 'edit_task': {
          ...
          App._renderTasks();
          App.showToast(`✏️ 할일 수정됨`,'success');
          break;
        }
        case 'delete_task': {
          if(!Auth.isLoggedIn()) break;
          ...
          if(!tid&&action.title){
            for(const [lId,list] of Object.entries(App.S.tasks||{})){
              const found=list.find(t=>t.title.includes(action.title)&&t.status==='needsAction');
              if(found){ tid=found.id; lid=lId; break; }
            }
          }
          if(!lid||!tid) break;
        ...
        case 'edit_checklist': {
          const items=Checklist.getItems();
          const it=items.find(i=>i.id===action.id||(action.title&&i.title.includes(action.searchTitle||action.title)));
          if(it){ if(action.newTitle) it.title=action.newTitle; if(action.dueDate!==undefined) it.dueDate=action.dueDate; Checklist.saveItems(items); Checklist.render(); App.showToast('✏️ 체크리스트 수정됨','success'); }
          break;
        }
        ...
        case 'toggle_checklist': {
          ...
          if(it){ it.done=action.done!==undefined?action.done:!it.done; Checklist.saveItems(items); Checklist.render(); }
          break;
        }
        ...
        case 'add_memo': {
          ...
          Memo.saveItems(items); Memo.render();
          App.showToast(`📝 메모: ${action.title}`,'success');
          break;
        }
        case 'edit_memo': {
          const items=Memo.getItems();
          const m=items.find(i=>i.id===action.id||(action.searchTitle&&i.title.includes(action.searchTitle)));
          if(m){ if(action.title) m.title=action.title; if(action.content!==undefined) m.content=action.content; m.updatedAt=new Date().toISOString(); Memo.saveItems(items); Memo.render(); App.showToast('✏️ 메모 수정됨','success'); }
          break;
        }
        case 'delete_memo': {
          const before=Memo.getItems();
          const after=before.filter(m=>!(m.id===action.id||(action.title&&m.title.includes(action.title))));
          if(after.length<before.length){ Memo.saveItems(after); Memo.render(); App.showToast('🗑 메모 삭제됨','success'); }
          break;
        }
        ...
        case 'delete_food': {
          ...
          if(dd[meal]){ dd[meal]=dd[meal].filter(f=>!f.name.includes(action.name||'__')); Diet.saveData(dd); Diet.render(); App.showToast('🗑 식단 삭제됨','success'); }
          break;
        }
        ...
        case 'complete_habit': {
          ...
          if(!chk.includes(action.habitId)){ chk.push(action.habitId); Habits.setChecked(chk); Habits.render(); App.showToast(`✅ 습관 완료`,'success'); }
          break;
        }
        case 'uncheck_habit': {
        ...
        case 'add_habit': {
          ...
          Habits.saveList(list); Habits.render();
          App.showToast(`✅ 습관 추가: ${action.name}`,'success');
          break;
        }
        case 'delete_habit': {
          const hl=Habits.getList();
          const after=hl.filter(h=>!(h.id===action.habitId||(action.name&&h.name.includes(action.name))));
          if(after.length<hl.length){ Habits.saveList(after); Habits.render(); App.showToast('🗑 습관 삭제됨','success'); }
          break;
        }
      }
  ...
  async _proactiveCheck() {
    ...
    if(hour>=17){ const plan=Fitness.PLAN[now.getDay()]; const chk=Fitness._checked(); if(plan.exercises.length>0&&chk.length===0) issues.push(`운동(${plan.name}) 미시작`); }
    if(hour>=15){ const t=Diet.totals(Diet.getData()); const g=Diet.getSettings(); if(t.cal<g.calorieGoal*0.35) issues.push(`칼로리 부족 (${t.cal}/${g.calorieGoal}kcal)`); }
    if(hour>=11){ const list=Habits.getList(); const chk=Habits.getChecked(); const miss=list.filter(h=>!chk.includes(h.id)); if(miss.length>=3) issues.push(`미완료 습관 ${miss.length}개`); }
    if(!issues.length) return;
    this._proactiveShown=true;
    try{
      const raw=await this._callAPI(key,`[분석] ${issues.join(', ')} — 1-2문장 격려 부탁해`);
      this._handleReply(raw);
  ...
  _buildCtx() {
    ...
    const pending=[];
    Object.entries(App.S.tasks||{}).forEach(([lid,list])=>list.filter(t=>t.status==='needsAction').forEach(t=>pending.push({id:t.id,title:t.title,listId:lid,due:t.due})));
...
{"response":"한국어 답변","actions":[],"tips":[]}

사용 가능한 actions (복수 가능):
{"type":"add_task","title":"","due":"YYYY-MM-DD or null","notes":"","listId":"(목록 id)"}
{"type":"edit_task","taskId":"id","listId":"id","title":"","notes":"","due":"YYYY-MM-DD or null"}
{"type":"delete_task","taskId":"id or null","listId":"id or null","title":"제목 일부"}
{"type":"add_event","summary":"","start":"2026-05-22T14:00:00+09:00","end":"2026-05-22T15:00:00+09:00","location":"","description":""}
{"type":"delete_event","eventId":"id or null","title":"제목 일부"}
{"type":"add_checklist","title":"","dueDate":"YYYY-MM-DD or null"}
{"type":"edit_checklist","id":"id or null","searchTitle":"찾을 제목","newTitle":"새 제목","dueDate":"YYYY-MM-DD or null"}
{"type":"delete_checklist","id":"id or null","title":"제목 일부"}
{"type":"toggle_checklist","id":"id or null","title":"제목 일부","done":true}
{"type":"add_memo","title":"","content":""}
{"type":"edit_memo","id":"id or null","searchTitle":"찾을 제목","title":"새 제목","content":"새 내용"}
{"type":"delete_memo","id":"id or null","title":"제목 일부"}
{"type":"log_food","meal":"아침|점심|저녁|간식","name":"","cal":0,"protein":0,"carb":0,"fat":0}
{"type":"delete_food","meal":"","name":"이름 일부"}
{"type":"complete_habit","habitId":"h1"}
{"type":"uncheck_habit","habitId":"h1"}
{"type":"add_habit","name":"","emoji":"⭐","days":[0,1,2,3,4,5,6]}
{"type":"delete_habit","habitId":"id or null","name":"이름 일부"}

식품 칼로리: 닭가슴살100g=165kcal/단31g, 달걀=77kcal/단6g, 흰밥200g=260kcal/탄57g, 삼겹살100g=331kcal/지28g, 치킨조각=213kcal/단18g, 김밥=420kcal, 아메리카노=5kcal, 프로틴=120kcal/단25g`;
  },
  ...
  _addMsg(type,text) {
    ...
    const d=document.createElement('div'); d.className=`j-msg ${isAI?'j-ai':'j-user'}`;
    const clean=esc(text).replace(/\n/g,'<br>');
    d.innerHTML=isAI?`<div class="j-avatar">⚡</div><div class="j-bubble${type==='tip'?' j-tip':''}">${clean}</div>`:`<div class="j-bubble j-bubble-user">${clean}</div>`;
